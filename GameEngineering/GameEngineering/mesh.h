...
struct Vertex {
    ...
    inline const float* getPosition() const { return p.getValues(); }
...
public:
    ...
    static Mesh makeRectangle(float x1, float y1, float x2, float y2) {
        ...
        // 批量计算边缘向量
        std::vector<vec4> edges1 = { v2 - v1 };
        std::vector<vec4> edges2 = { v4 - v1 };

        // 批量叉积计算法线
        std::vector<vec4> normals = helper.crossProductParallel(edges1, edges2);

        helper.normalizeVectorsParallel(normals);


        // 使用归一化后的法线
        vec4 normal = normals[0];
    ...
    static Mesh makeCube(float size) {
        ...
        auto worker = [&](int start, int end) {
            for (int i = start; i < end; ++i) {
...

                {
                    ...
                    mesh.addVertex(positions[v0], normal);
                    mesh.addVertex(positions[v1], normal);
                    mesh.addVertex(positions[v2], normal);
                    mesh.addVertex(positions[v3], normal);
                }
        ...
        int facesPerThread = 6 / numThreads;
        for (int t = 0; t < numThreads; ++t) {
            int start = t * facesPerThread;
            int end = (t == numThreads - 1) ? 6 : start + facesPerThread;
    ...
    static Mesh makeSphere(float radius, int latitudeDivisions, int longitudeDivisions) {
        ...
        int numThreads = std::thread::hardware_concurrency();
        std::vector<std::thread> threads;
        std::mutex vertexMutex; // 用于保护共享的 vertices

        auto generateVertices = [&](int start, int end) {
        ...
        int latitudesPerThread = latitudeDivisions / numThreads;
        for (int t = 0; t < numThreads; ++t) {
            int start = t * latitudesPerThread;
            int end = (t == numThreads - 1) ? latitudeDivisions + 1 : start + latitudesPerThread;
            threads.emplace_back(generateVertices, start, end);
        }
        ...
        auto generateTriangles = [&](int start, int end) {
            for (int lat = start; lat < end; ++lat) {
                for (int lon = 0; lon < longitudeDivisions; ++lon) {
                    int v0 = lat * (longitudeDivisions + 1) + lon;
                    int v1 = v0 + 1;
        ...
        for (int t = 0; t < numThreads; ++t) {
            int start = t * latitudesPerThread;
            int end = (t == numThreads - 1) ? latitudeDivisions : start + latitudesPerThread;
            threads.emplace_back(generateTriangles, start, end);
        }
