...
public:
    ...
    vec4 operator * (const vec4& v) const {
        __m128 vec = _mm_loadu_ps(v.getV()); // 加载 vec4 到 SIMD 寄存器
        vec4 result;

        for (int i = 0; i < 4; ++i) {
            __m128 row = _mm_loadu_ps(&a[i * 4]); // 加载矩阵的行
            __m128 res = _mm_dp_ps(row, vec, 0xF1); // 点积
            _mm_store_ss(&result[i], res); // 只存储点积的结果
        }
        return result;
    }
    ...
    matrix operator * (const matrix& mx) const {
        ...
        for (int row = 0; row < 4; ++row) {
            __m128 rowVec = _mm_loadu_ps(&a[row * 4]); // 加载当前行
            for (int col = 0; col < 4; ++col) {
                __m128 colVec = _mm_set_ps(mx.a[3 * 4 + col], mx.a[2 * 4 + col], mx.a[1 * 4 + col], mx.a[0 * 4 + col]);
                __m128 res = _mm_dp_ps(rowVec, colVec, 0xF1);
                _mm_store_ss(&ret.a[row * 4 + col], res);
            }
        }
        return ret;
    }
...
private:
    ...
    void identity() {
        __m128 row1 = _mm_set_ps(0, 0, 0, 1);
        __m128 row2 = _mm_set_ps(0, 0, 1, 0);
        __m128 row3 = _mm_set_ps(0, 1, 0, 0);
        __m128 row4 = _mm_set_ps(1, 0, 0, 0);

        _mm_storeu_ps(&a[0], row1);
        _mm_storeu_ps(&a[4], row2);
        _mm_storeu_ps(&a[8], row3);
        _mm_storeu_ps(&a[12], row4);
    }
